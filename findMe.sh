...
if [[ "$locationStatus" -eq 0 ]]; then
 notify_post disableGPS
fi

if [[ "$airplaneStatus" -eq 1 ]]; then
 notify_post enableAirplane
fi

if [[ "$dataStatus" -eq 0 ]]; then
   notify_post disableData
fi
...
do
    ...
    case "$opt" in
        ...
        "-r" )
           ...
           RUNS="$1";  
           if [[ $RUNS -lt 1 ]]; then 
echo "Less than one run? Okay..." >> $TMPLOG;
cleanUp;
fi
shift;;
        "-o" )
...
notify_post enableData
notify_post enableGPS
notify_post disableAirplane
OUTPUT=loc.tmp
PROGRAM=./LcMe
